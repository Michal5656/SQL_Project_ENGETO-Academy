WITH data_vse AS (
    SELECT 
        ...
        s.hdp
    FROM "t_Michal_Čulík_project_sql_primary_final" p
    JOIN "t_Michal_Čulík_project_sql_secondary_final" s
        ON p.rok = s.rok::int 
        AND s.země = 'Czech Republic'
...
SELECT 
    rok,
    narust_hdp,
    LAG(narust_hdp) OVER (ORDER BY rok) AS narust_hdp_loni,
    narust_mzdy,
    narust_ceny
